              ...
              return (
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
                  ...
                  <form onSubmit={onUploadMom} className="space-y-3.5 text-xs font-semibold text-slate-700">
                    {/* Scholar name and Meeting Type hidden elements to preserve handler payload */}
                    <input type="hidden" name="scholarName" value={s.name} />
                    <input type="hidden" name="meetingType" value={phase + " Meeting"} />

                    <div>
                      <label className="block mb-1 text-slate-600 font-bold">Meeting Date</label>
                      <input type="date" name="meetingDate" defaultValue={new Date().toISOString().split('T')[0]} className="w-full border border-slate-200 p-2.5 rounded-xl bg-slate-50 text-xs focus:outline-none focus:border-dsu-maroon font-bold" required />
                    </div>
                      ...
                      return (
                        <div className="space-y-4 text-xs text-slate-700">
                          <div className={`p-4 rounded-xl border ${isCompleted ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                            <div className="flex items-center gap-2">
                              {isCompleted ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : <Clock className="w-4.5 h-4.5 text-slate-505" />}
