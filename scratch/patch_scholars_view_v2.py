              ...
              return (
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
                  ...
                  <form onSubmit={onUploadMom} className="space-y-3.5 text-xs font-semibold text-slate-700">
                    <div>
                      <label className="block mb-1 text-slate-600 font-bold">Scholar</label>
                      <select name="scholarName" defaultValue={s.name} className="w-full border border-slate-200 p-2.5 rounded-xl bg-slate-50 text-xs focus:outline-none focus:border-dsu-maroon font-bold" required>
                        {scholars.map((sch, i) => (
                          <option key={i} value={sch.name}>{sch.name} &mdash; {sch.reg}</option>
                        ))}
                      </select>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    {phase === 'Synopsis' && (() => {
                      ...
                      return (
                        <div className="space-y-4 text-xs text-slate-700">
                          <div className={`p-4 rounded-xl border ${isCompleted ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
                            <div className="flex items-center gap-2">
